...
from windows import FilmList, AdminFilmList
...
class Ui_Form_2(object):
    ...
    def open_films(self):
        if(self.user_check == 1):
            self.new_window = AdminFilmList()
            print("Таблица для Админа")
        else: self.new_window = FilmList()
    ...
    def open_serials(self):
        ...
        exeption.setWindowTitle("Sorry but...")
        exeption.setText("Sorry but...")  # всплывающее окно
    ...
    def open_game(self):
        ...
        random_film = session.query(Film).all()  # ищем рандомный фильм
        a = random.choice(random_film)
        print(a.film_title)
        exeption.setText(a.film_title)  # всплывающее окно
        exeption.setIcon(QMessageBox.Information)
        exeption.setStandardButtons(QMessageBox.Ok)
        exeption.exec_()
