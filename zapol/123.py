from DATAbase import Film, Actor, Category,Admin
...
# добавим Админа
#session.add(Admin(admin_login = "Admin",admin_password = 123456789))
#session.commit()
#session.close()
...
while i in range(len(new)):
    ...
    new_film = Film(film_title=new[i], film_duration=new[i + 2], category_id=category[0].category_id, film_release_date=str(new[i+3]))
